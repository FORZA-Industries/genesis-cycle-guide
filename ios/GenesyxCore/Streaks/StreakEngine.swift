// StreakEngine.swift
// Target path in the iOS repo: Sources/GenesyxCore/Streaks/StreakEngine.swift
//
...
// MARK: - CalendarDate
// INTEGRATE: GenesyxCore may already define CalendarDate — if so, delete this block
// and keep the extension methods below (weekday/addingDays) wherever they fit.

/// A timezone-safe, time-of-day-free calendar date. Comparable and Hashable so it
/// can key dictionaries and sets deterministically in tests.
public struct CalendarDate: Hashable, Comparable, CustomStringConvertible {
    public let year: Int
    ...
    public func asDate(calendar: Calendar = .current) -> Date {
        calendar.date(from: DateComponents(year: year, month: month, day: day))!
    }

    public func addingDays(_ days: Int, calendar: Calendar = .current) -> CalendarDate {
        CalendarDate(calendar.date(byAdding: .day, value: days, to: asDate(calendar: calendar))!,
                     calendar: calendar)
    }

    /// ISO weekday: 1 = Monday … 7 = Sunday (weeks are Mon–Sun per the spec).
    public var isoWeekday: Int {
        let gregorianWeekday = Calendar(identifier: .iso8601)
            .component(.weekday, from: asDate()) // 1 = Sunday … 7 = Saturday
        return gregorianWeekday == 1 ? 7 : gregorianWeekday - 1
    }

    /// The Monday of this date's Mon–Sun week.
    public var startOfWeek: CalendarDate {
...
// MARK: - Input protocol
// INTEGRATE: conform the existing DailyLog model to this (one extension, no model change):
//
//   extension DailyLog: StreakLoggable {
//       public var hasAnyEntry: Bool {
//           waterMl > 0 || mood != nil || energy != nil || !symptoms.isEmpty
//               || sleepMinutes != nil || !supplements.isEmpty
//               || !(notes ?? "").isEmpty
//       }
//   }
